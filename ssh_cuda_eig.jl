...
function main()
    ...
    if CUDA.functional()
        println("\nCUDA-enabled GPU found. Running eigenvalue calculation on the GPU...")

        H_gpu = cu(H)
        H_gpu_data = copy(parent(H_gpu))

        eigenvalues_gpu = CUSOLVER.syevd!('N', 'U', H_gpu_data)

        eigenvalues = Array(eigenvalues_gpu)

        println("GPU calculation complete.")
    else
        println("\nNo functional CUDA device found. Running eigenvalue calculation on the CPU...")

        eigenvalues = eigvals(H)

        println("CPU calculation complete.")
    end

    output_filename = "/home/antpc/Downloads/eigenvalues_L$(L)_Np$(Np)_gpu.txt"
    writedlm(output_filename, eigenvalues)
    println("\nFull set of eigenvalues saved to '$(output_filename)'")

    println("\n--- Results ---")
    num_eigenvalues_to_print = min(10, length(eigenvalues))
    @printf "Lowest %d Eigenvalues:\n" num_eigenvalues_to_print
    for i in 1:num_eigenvalues_to_print
        @printf "  E_%d = %.6f\n" (i-1) eigenvalues[i]
    end
end
